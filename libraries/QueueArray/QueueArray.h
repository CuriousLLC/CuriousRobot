 ...
 *  ---
 * 
 *  Version 1.0.1 Ryan Day <ryan@ryanday.net>
 *      - Modifications to reduce size
 *      - Don't resize contents, use fixed size upfront. Reduces allocations
 *        and increases speed.
 *
 *  Version 1.0
 *
...
// add an item to the queue.
template<typename T>
int QueueArray<T>::enqueue (const T i) {
  // check if the queue is full.
  if (isFull ())
      return -1;
